...
process_py() {
    local import_sorted, black_config
...
process_sh() {
    local language_variant, indent_size, binary_start, ident_case, space_redirect, keep_padding, minify
...
for file in "$@"; do
    ...
    if [ -z "$(cat "$file")" ]; then
        ...
        continue
    elif $process "$file" && [ -n "$__return" ]; then
        formatted="$__return"
    else
